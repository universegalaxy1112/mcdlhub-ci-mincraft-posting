...
class Home_controller extends Home_Core_Controller
{
    ...
    {
        $this->load->model("cron_page_model");
        $main_categories = [2, 3, 5, 6, 50, 333];
        $categories_to_retrieve = [2, 3, 5, 6, 449, 431, 432, 433, 484];
        $tags_to_categorise = [10, 11, 13, 14, 15, 16, 17, 22, 27, 28, 31, 45, 123, 279, 322, 405, 419, 435, 440, 458, 479, 483, 411];
        //$types_to_retrieve = [0 => 'posts', 50 => 'skin', 333 => 'server', 431 => 'addon', 433 => 'modpe', 6 => 'seed', 449 => 'skinpack', 5 => 'texturepack', 435 => 'world', 484 => 'script'];
        //$types_to_retrieve = [333 => 'server'];
        $types_to_retrieve = [0 => 'posts', 333 => 'server', 6 => 'seed'];

        foreach ($types_to_retrieve as $category_id => $type_to_retrieve) {
            try {
                // get current page
                $page_result = $this->cron_page_model->getPage($type_to_retrieve);
                $page = isset($page_result) ? $page_result : 1;


                // update the current page
                $this->cron_page_model->updatePage($page + 1, $type_to_retrieve);

                // get posts
                if ($type_to_retrieve === 'posts') {
                    $query = array("page" => $page, "per_page" => 100, "categories" => $categories_to_retrieve, "orderby" => 'modified', "order" => 'desc');
                } else {
                    $query = array("page" => $page, "per_page" => 100, "orderby" => 'modified', "order" => 'desc');
                }

                $curl_url = 'https://mcpedl.com/wp-json/wp/v2/' . $type_to_retrieve;

                $posts = json_decode($this->callCurl($curl_url, $query), true);
                $response_code = isset($posts['code']) ? $posts['code'] : 'success';
                if ($response_code === 'success') {
                    foreach ($posts as $post) {
                        try {
                            ...
                            $data['title_slug'] = isset($post['slug']) ? $post['slug'] : [];
                            $data['created_at'] = isset($post['modified']) ? $post['modified'] : date('Y-m-d h:i:s a', time());
                            $data['title'] = isset($post['title']['rendered']) ? $post['title']['rendered'] : '';
                            $summary = isset($post['excerpt']['rendered']) ? $post['excerpt']['rendered'] : '';
                            $data['summary'] = str_replace(['<p>', '</p>'], '', $summary);
                            $data['is_slider'] = true;
                            ...
                            $table = '';
                            if ($type_to_retrieve === 'server') {
                                $scrap_url = 'https://mcpedl.com/servers/' . $data['title_slug'];
                                $html = file_get_contents($scrap_url);
                                $dom_server = new DOMDocument;
                                @$dom_server->loadHTML('<?xml encoding="utf-8" ?>' . $html);
                                $tables = $dom_server->getElementsByTagName('table');
                                foreach ($tables as $t) {
                                    if ($t->getAttribute('class') === 'srv-info') {
                                        $table = $dom_server->saveHTML($t);
                                        break;
                                    }
                                }
                            }
                            $content = isset($post['content']['rendered']) ? $post['content']['rendered'] : '';
                            $content = $table . '<br />' . str_replace('mcpedl.com/?s=', 'mcdlhub.com/profile/', $content);
                            if ($content) {
                                $dom = new DOMDocument;
                                ...
                                if ($type_to_retrieve === 'posts') {
                                    $hasInTags = false;
                                    foreach ($tags as $tag) {
                                        if (in_array($tag, $tags_to_categorise, true)) {
                                            $hasInTags = true;
                                            $this->post_admin_model->add_external_post($tag, $data);
                                            break;
                                        }
                                    }
                                    if ($hasInTags === false) {
                                        foreach ($categories as $category) {
                                            if (count($categories) === 1 || !in_array($category, $main_categories, true)) {
                                                $this->post_admin_model->add_external_post($category, $data);
                                                break;
                                            }
                                        }
                                    }
                                } else {
                                    $this->post_admin_model->add_external_post($category_id, $data);
                                    echo $category_id;
                                }
                    ...
                    $this->cron_page_model->updatePage(1, $type_to_retrieve);
                }
            } catch (Exception $e) {
                var_dump($e);
            }
