...
class Post_admin_model extends CI_Model
{
    ...
    public function add_external_post($category_id, $data) {
        ...
        $ext = ltrim($ext, '/');
        $data['image_mime'] = $ext;
        $this->db->where('title_slug', $data['title_slug']);
    ...
    public function get_total_post_views() {
        $this->db->select_sum('pageviews');
        $this->db->from('posts');
        $query = $this->db->get();
        return $query->row()->pageviews;
    }
