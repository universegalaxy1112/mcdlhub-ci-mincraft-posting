...
class File_model extends CI_Model
{
   ...
   */
    ...
    public function upload_audio()
    {
        ...
        if (!empty($audio)) {
            $data["audio_name"] = trim($audio['file_name']);
            $data["download_button"] = $this->input->post('download_button', true);
            $data["audio_path"] = $audio['file_path'];
    ...
    function insert_audio($data)
    {
        ...
        $download_button = 0;
        if (!empty($data["download_button"])) {
            $download_button = 1;
        }

        // Check for errors
        if (!mysqli_connect_errno()) {
            $mysqli->query("INSERT INTO `audios` (`audio_name`,`audio_path`,`download_button`,`user_id`) VALUES (" . $ci->db->escape($audio_name) . "," . $ci->db->escape($audio_path) . "," . $download_button . "," . $this->auth_user->id . ");");
        }
        // Close the connection
        $mysqli->close();
    }
