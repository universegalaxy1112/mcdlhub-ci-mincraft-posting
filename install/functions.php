...
{
    $url = "https://codingest.net/api/verify-varient-license?license_code=" . $license_code . "&domain=" . $current_url;
    $ch = curl_init();
    ...
    curl_close($ch);
    if (empty($response)) {
        $url = "http://codingest.net/api/verify-varient-license?license_code=" . $license_code . "&domain=" . $current_url;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $response = curl_exec($ch);
